...
MODO_JOGO:       WORD 0       ; indica se o jogo está parado (0 - não, 1 - sim)
...
inicio:
    MOV  R1, JOGAVEL
    MOV  [MODO_JOGO], R1        ; inicializa MODO_JOGO a jogavel
    MOV  [APAGA_AVISO], R1             
    MOV  [APAGA_ECRÃ], R1	        
    MOV  R1, 1                   
...
fim_jogo:
    ...
    MOV R5, 16                     ; número de ciclos
    MOV R2, 0                      ; inicializa o índice da tabela
    MOV R9, 0                      ; inicializa o valor a colocar na tabela
    CALL apaga_entradas_tabela
    MOV R5, 5                      ; número de ciclos
    MOV R2, 4                      ; inicializa o índice da tabela
    CALL apaga_entradas_tabela2
    CALL apaga_entradas_sonda
    JMP  recomeca
...
recomeca:
    ...
    MOV R8, [tecla_carregada]      ; bloqueia neste LOCK até uma tecla ser 
                                   ; carregada
    MOV R9, TECLA_E                ; verifica se a tecla premida corresponde a tecla D
    CMP R8, R9
    JZ  volta_ecra_inicio
    JMP recomeca
...
pausa:
    ...
    MOV R8, [tecla_carregada]      ; bloqueia neste LOCK até uma tecla ser 
                                   ; carregada
    MOV R9, TECLA_D                ; verifica se a tecla premida corresponde a tecla D
    CMP R8, R9
    JZ  restart
    ;MOV R9, TECLA_E               ; verifica se a tecla premida corresponde a tecla E
    ;CMP R8, R9
    ;JZ  fim_jogo
    JMP pausa


apaga_entradas_tabela:
    MOV R1, TAB_ASTEROIDE     ; Endereço inicial da tabela                
    MOV [R1+R2], R9           ; reinicializa a linha do asteróide
    ADD R2, 2                 ; passa para a próxima linha
    SUB R5, 1                 ; decrementa o contador de linhas
    CMP R5, 0                 ; verifica se já foram reinicializadas todas
    JNZ apaga_entradas_tabela ; as linhas da tabela
    RET

apaga_entradas_tabela2:
   MOV R1, PAR_COLUNA_DIRECAO
    MOV [R1+R2], R9
    ADD R2, 6
    SUB R5, 1
    CMP R5, 0
    JNZ apaga_entradas_tabela2
    RET
...
verifica_colisao:
    ...
    ADD  R10, 2               ; atualiza índice da tab para coluna colisão
    MOV  R9, [R2+R10]         ; acede à posição da coluna de colisão
    SUB  R6, R8               
    SUB  R7, R9             
    ADD  R6, R7               ; se for zero, houve colisão   
    MOV  R3, R6               ; copia resultado da subtração
    POP  R10
    POP  R9 
    POP  R8
    POP  R7
