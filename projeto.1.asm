...
SONDA_DISPARADA: WORD 0       ; indica se a sonda foi disparada
...
;JOGO_INICIO:  WORD 0   ; caso esteja fora de jogo, 1 em estado jogavel
...
lock_energia: 
    LOCK 0              ; LOCK para o processo energia comunicar aos restantes 
                        ; processos o valor da energia
...
comandos:
    ...
    ;CMP  R8, TECLA_D             ; ao comando  que coloca o jogo em pausa
    ;JZ beyond_mars

    MOV  R9, TECLA_E
    CMP  R8, R9             ; ao comando  que termina o jogo
    JZ fim_jogo
    JMP  comandos
...
desenha_sonda:
    MOV  R1, 1                 
    MOV  [SONDA_DISPARADA], R1      ; sonda disparada
    MOV  R1, 1              
    ...
    CALL escreve_pixel
    MOV  [lock_energia], R0         ; bloqueia neste LOCK
...
fim_jogo:
MOV [APAGA_ECRÃ], R1
MOV R1, 2                              ; escolhe a imagem correta 
MOV [SELECIONA_FUNDO], R1             ; seleciona o fundo
...
ciclo_energia:
    MOV  R1, [SONDA_DISPARADA]   
    CMP  R1, 1  
    JZ   sonda_unlock
    MOV  R2, evento_int_energia   
    MOV  R0, [R2]                 ; lê o LOCK
    MOV  R1, [ENERGIA]
...
sonda_unlock:
    MOV  R0, [lock_energia]       ; lê o LOCK
    MOV  R1, 0
    MOV  [SONDA_DISPARADA], R1    ; sonda já foi disparada
    JMP  ciclo_energia
...
rot_int_2:
	...
	MOV	 [R1], R0	                 ; desbloqueia processo energia
    POP  R1
	RFE 
...
MOV [APAGA_ECRÃ], R1
MOV R1 , 3                            ; escolhe a imagem correta 
MOV [SELECIONA_FUNDO], R1             ; seleciona o fundo
